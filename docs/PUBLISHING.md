...
| VS Code extension | `dist/chronovault-1.0.0.vsix` (stage: `vscode-extension/chronovault-1.0.0.vsix`) |
| IntelliJ plugin | `dist/chronovault-intellij-1.0.0.zip` (stage: `intellij-plugin/build/distributions/...`) |
| Release report | `dist/RELEASE-REPORT.md` and `docs/release/RELEASE-REPORT.md` |

Inspect before publishing:

```bash
unzip -l vscode-extension/chronovault-1.0.0.vsix
unzip -l intellij-plugin/build/distributions/chronovault-intellij-1.0.0.zip
unzip -p intellij-plugin/build/distributions/chronovault-intellij-1.0.0.zip \
  chronovault-intellij/lib/chronovault-intellij-1.0.0.jar META-INF/plugin.xml
```
...
3. Login:
   ...
   npx -y @vscode/vsce package      # -> chronovault-1.0.0.vsix
   ```
4. [Optional] Open VSX (open-source overlay marketplace):
   ```bash
   npx -y @vscode/vsce publish --packagePath chronovault-1.0.0.vsix
   ```
