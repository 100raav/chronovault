...
| VS Code extension | `dist/chronovault-1.0.1.vsix` (stage: `vscode-extension/chronovault-1.0.1.vsix`) |
| IntelliJ plugin | `dist/chronovault-intellij-1.0.1.zip` (stage: `intellij-plugin/build/distributions/...`) |
| Release report | `dist/RELEASE-REPORT.md` and `docs/release/RELEASE-REPORT.md` |

Inspect before publishing:

```bash
unzip -l vscode-extension/chronovault-1.0.1.vsix
unzip -l intellij-plugin/build/distributions/chronovault-intellij-1.0.1.zip
unzip -p intellij-plugin/build/distributions/chronovault-intellij-1.0.1.zip \
  chronovault-intellij/lib/chronovault-intellij-1.0.1.jar META-INF/plugin.xml
```
...
3. Login:
   ...
   npx -y @vscode/vsce package      # -> chronovault-1.0.1.vsix
   ```
4. [Optional] Open VSX (open-source overlay marketplace):
   ```bash
   npx -y @vscode/vsce publish --packagePath chronovault-1.0.1.vsix
   ```
